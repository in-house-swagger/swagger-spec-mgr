...
public class SwaggerSpecUtilsTest {
    ...
    private static final String DIR_BASE = "src/test/scripts/util/" + SwaggerSpecUtilsTest.class.getSimpleName();
    ...
    @SuppressWarnings("unchecked")
    @Test
    public final void testSplitAndMerge() throws JsonParseException, JsonMappingException, IOException {
        // -----------------------------------------------------------------------------------------
        // 設定
        // -----------------------------------------------------------------------------------------
        ...
        // 分割除外パス 正規表現リスト
        final List<String> ignorePathList = Arrays.asList(new String[] {
            "/info/.*",
            "/tags/.*",
            "/schemas/.*",
            "/securityDefinitions/.*",
        ...
        // merge
        final String mergedFilePath = SwaggerSpecUtils.writeMerged(DIR_OUTPUT_SPLIT, DIR_OUTPUT_MERGED, specId);
        // parse
