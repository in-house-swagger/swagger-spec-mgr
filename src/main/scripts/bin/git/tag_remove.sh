...
#--------------------------------------------------------------------------------
function usage() {
  echo "Usage: $(basename $0) TO_TAG [USER]" >&2
  exit ${EXITCODE_ERROR}
}
...
if [ "${has_origin}" != "true" ]; then
  # 存在しない場合、localのみ
  git.tag_remove_local "${dir_repo}" "${to_tag}"                                              2>&1 | tee -a "${PATH_LOG}"
else
  # 存在する場合、pushあり
  git.tag_remove "${dir_repo}" "${to_tag}"                                                    2>&1 | tee -a "${PATH_LOG}"
fi
ret_code=${PIPESTATUS[0]}

if [ ${ret_code} -ne ${EXITCODE_SUCCESS} ]; then
  git.common.exit_script ${EXITCODE_ERROR} "タグの削除 でエラーが発生しました。"
fi
